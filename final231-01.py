...
class FallDetectionMode(Enum):
    ...
    SIMPLE_OR = auto()        # F4 - Velocity OR Torso OR Leg (1 of 3)
    MAJORITY = auto()         # F5 - 2 of 3 must trigger (more robust)
    ACCELERATION = auto()     # F6 - How fast velocity increases
    VELOCITY_AND_ANGLE = auto() # F7 - Both velocity AND angle (strictest)
    TORSO_FLOOR = auto()      # F8 - Torso distance to floor (3D)
    HAND_POSITION = auto()    # F9 - Hands lower than normal
    HAND_FEET_OVERLAP = auto() # F10 - Hands near feet level
    BODY_RATIO = auto()       # F11 - Body aspect ratio change
    COMBINED = auto()         # F12 - Weighted combination
    ...
    def get_name(cls, mode):
        names = {
            ...
            cls.SIMPLE_OR: "SIMPLE_OR (1 van 3)",
            cls.MAJORITY: "MAJORITY (2 van 3)",
...
class FallDetector:
    ...
    def _evaluate_fall(self, state: PersonFallState) -> bool:
        ...
        elif self.active_mode == FallDetectionMode.MAJORITY:
            # MAJORITY: 2 of 3 must trigger (more robust, fewer false positives)
            vel_triggered = scores.get(FallDetectionMode.VELOCITY, 0) > 0.7
            torso_triggered = scores.get(FallDetectionMode.TORSO_ANGLE, 0) > 0.7
            leg_triggered = scores.get(FallDetectionMode.LEG_ANGLE, 0) > 0.7
            
            triggered_count = sum([vel_triggered, torso_triggered, leg_triggered])
    ...
    for pid, pdata in tracker.persons.items():
        ...
        if in_roi:
            ...
            if fall_state and fall_state.is_calibrated:
                ...
                if active_mode in (FallDetectionMode.SIMPLE_OR, FallDetectionMode.MAJORITY):
                    # Show all 3 scores for OR/MAJORITY mode
                    ...
                    # Show with checkmarks for triggered
                    v_mark = "✓" if vel_t else ""
                    t_mark = "✓" if torso_t else ""
                    l_mark = "✓" if leg_t else ""
                    
                    cv2.putText(overlay, f"V:{vel:.1f}{v_mark} T:{torso:.1f}{t_mark} L:{leg:.1f}{l_mark} ({count}/3)", 
                                (x1, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, score_color, 1)
                    y_offset += 15
...
def main():
    ...
    # Start with MAJORITY mode (recommended - 2 of 3)
    fall_detector.set_mode(FallDetectionMode.MAJORITY)
    ...
    print("  - F4 = SIMPLE_OR (1 van 3 triggert)")
    print("  - F5 = MAJORITY ★ AANBEVOLEN (2 van 3 triggeren)")
    print("  - F6 = ACCELERATION")
    print("  - F7 = VELOCITY+ANGLE (beide)")
    print("  - F8 = TORSO_FLOOR (3D)")
    print("  - M = Cycle modes")
